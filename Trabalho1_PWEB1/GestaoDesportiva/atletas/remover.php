...
<head>
    <meta charset="UTF-8">
    <title>Desassociar Atleta</title>
...
<section role="main" class="content-body">
            <header class="page-header">
                <h2>Desassociar Atletas</h2>
                <div class="right-wrapper text-right">
                    <ol class="breadcrumbs">
                        <li><a href="../inicio.php"><i class="fa fa-home"></i></a></li>
                        <li><span>Página inicial / Atletas / Desassociar Atletas</span></li>
                    </ol>
                </div>
            </header>
    <section class="card">
        <div class="card-body">
            <form method="post" onsubmit="return confirm('Tem a certeza que pretende desassociar este atleta?');">
                <div class="form-group row">
                    <label class="col-sm-4 col-form-label">Selecione o atleta:</label>
                    <div class="col-sm-8">
                        <select name="id_atleta" class="form-control" required>
                            <option value="">-- Escolher --</option>
                            <?php
                            $res = mysqli_query($link, "
                                SELECT a.id_atleta, u.nome AS nome_atleta, m.nome AS modalidade
                                FROM atletas a
                            ...
                            while ($row = mysqli_fetch_assoc($res)) {
                                echo "<option value='{$row['id_atleta']}'>[{$row['id_atleta']}] {$row['nome_atleta']} - {$row['modalidade']}</option>";
                            }
                            ?>
                        </select>
                    </div>
            ...
            if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["id_atleta"])) {
                ...
                $del = mysqli_query($link, "DELETE FROM atletas WHERE id_atleta = $id");
                if ($del) {
                    echo "<p class='alert alert-success mt-3'>Atleta desassociado com sucesso. A página será atualizada...</p>";
                    echo "<script>setTimeout(() => location.href='remover.php', 3000);</script>";

                    $acao = $_SESSION["nome"] . " desassociou o atleta: {$info['nome_atleta']} ({$info['modalidade']})";
                    ...
                    echo "<p class='alert alert-danger mt-3'>Erro ao remover atleta.</p>";
                }
            }
            ?>
        </div>
