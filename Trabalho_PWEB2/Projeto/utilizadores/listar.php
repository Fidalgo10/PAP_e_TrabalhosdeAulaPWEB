...
$sql = "SELECT id, nome, email, telefone, perfil FROM utilizadores ORDER BY id ASC";
...
<head>
    <meta charset="utf-8">
    <title>Listar Utilizadores - Gestor de Tarefas</title>
    ...
    <div class="sidebar pe-4 pb-3">
        <nav class="navbar bg-light navbar-light">
            <a href="../index.php" class="navbar-brand mx-4 mb-3">
                <img src="../img/Instituto_Superior_de_Engenharia_de_Coimbra_logo.png" alt="Logo" style="width: 80%;">
            </a>
            <div class="navbar-nav w-100">
                <a href="../index.php" class="nav-item nav-link">
                    <i class="fa fa-tachometer-alt me-2"></i>Dashboard
                </a>
                <a href="#tarefasSubmenu" class="nav-item nav-link d-flex justify-content-between align-items-center"
                ...
                <div class="collapse show ps-4" id="utilizadoresSubmenu">
                    ...
                    <a href="listar.php" class="nav-item nav-link active">Listar Utilizadores</a>
                    <a href="remover.php" class="nav-item nav-link">Eliminar Utilizador</a>
                </div>
                <a href="../logs.php" class="nav-item nav-link"><i class="fa fa-history me-2"></i>Logs</a>
            ...
            <div class="row">
                <div class="col-12">
                    <div class="d-flex justify-content-between align-items-center mb-3">
                        ...
                        <a href="inserir.php" class="btn <?= ($perfil == 'administrador') ? 'btn-danger' : 'btn-primary' ?>">
                            <i class="fa fa-plus me-2"></i> Criar Novo Utilizador
                        </a>                    
                    </div>
                    ...
                    <div class="d-flex justify-content-between align-items-center mb-3">
                        ...
                        <button id="exportPDF" class="btn btn-danger"><i class="fa fa-file-pdf me-2"></i>Exportar PDF</button>
                    </div>

                    <div class="bg-light rounded p-4">
                        <div class="table-responsive">
                            <table class="table table-striped table-bordered" id="utilizadoresTable">
                                <thead class="bg-danger text-white">
                                    <tr>
                                        <th data-type="string">ID <span class="sort-arrow"></span></th>
                                        <th data-type="string">Nome <span class="sort-arrow"></span></th>
                                        <th data-type="string">Email <span class="sort-arrow"></span></th>
                                        <th data-type="string">Telefone <span class="sort-arrow"></span></th>
                                        <th data-type="string">Perfil <span class="sort-arrow"></span></th>
                                        <th>Ações</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <?php while ($row = mysqli_fetch_assoc($result)) {
                                        $perfilExib = ($row['id'] == 0) ? "Administrador Master" : $row['perfil']; ?>
                                        <tr>
                                            <td><?= $row["id"] ?></td>
                                            <td><?= htmlspecialchars($row["nome"]) ?></td>
                                            <td><?= htmlspecialchars($row["email"]) ?></td>
                                            <td><?= htmlspecialchars($row["telefone"]) ?></td>
                                            <td><?= htmlspecialchars($perfilExib) ?></td>
                                            <td>
                                                <?php if ($_SESSION['iduser'] == 0 || $row['perfil'] != 'administrador'): ?>
                                                    <a href="editar.php?id=<?= $row["id"] ?>" class="btn btn-sm btn-danger"><i class="fa fa-edit"></i> Editar</a>
                                                <?php else: ?>
                                                    <span class="text-muted">[Admin Master]</span>
                                                <?php endif; ?>
                                            </td>
                                        </tr>
                                    <?php } ?>
...
document.getElementById('searchInput').addEventListener('keyup', function() {
    ...
    const rows = document.querySelectorAll('#utilizadoresTable tbody tr');
    rows.forEach(row => {
...
// 📄 Exportar PDF com logo e nome dinâmico
document.getElementById('exportPDF').addEventListener('click', function() {
    const table = document.getElementById('utilizadoresTable'); // Muda para a tabela correta

    // Clonar tabela para não alterar a original
    const cloneTable = table.cloneNode(true);

    // Remover última coluna (ex: "Ações")
    ...
    inputName.value = 'lista_de_utilizadores.pdf'; // muda para cada página
    ...
    inputTitle.value = 'Exportação da Tabela de Utilizadores'; // muda conforme a página
...
// 🔀 Ordenação clicável
document.querySelectorAll('#utilizadoresTable th[data-type]').forEach((header, index) => {
    let asc = true;
